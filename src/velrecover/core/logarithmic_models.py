...
def custom_logarithmic_interpolate(CDP, TWT, VEL, CDP_grid, TWT_grid, CDP_range, TWT_range, 
    ...
    for i in range(VEL_grid.shape[1]):  # For each CDP
        VEL_grid[:, i] = logarithmic_model(TWT_range, v0, k)
...
def best_logarithmic_interpolate(CDP, TWT, VEL, CDP_grid, TWT_grid, CDP_range, TWT_range, 
...
def best_logarithmic_fit(text_file_path=None, segy_file_path=None, 
    ...
    Args:
        ...
        status_callback: Function for updating progress
        cancel_check: Function for checking if operation should be cancelled
        **kwargs: Additional parameters (cdp, twt, vel, console can be passed here)
